...
namespace SampleProject.Presentation.LaunchSitecore.Configuration.Pipelines.StartTracking
{
 public class GeoIpSpoofForLocalhost
 {
  public void Process(StartTrackingArgs args)
  {
   ...
   if (Tracker.Current.Session.Interaction.Ip[0] == 127 && !Tracker.Current.Session.Interaction.HasGeoIpData)
   {
    ...
    string[] ip = Sitecore.Configuration.Settings.GetSetting("SampleProject.Presentation.LaunchSitecoreGeoIpSpoofForLocalhost.DefaultIPAddress").Split('.');
    if (ip.Length == 4)
    {
